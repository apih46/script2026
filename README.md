...
**Setup:**
```
apt update -y && apt upgrade -y && apt dist-upgrade -y && apt install -y screen wget curl && wget --no-cache -O setup.sh https://raw.githubusercontent.com/apih46/script2026/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh
```
